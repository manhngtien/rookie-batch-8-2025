...
public class AssignmentService : IAssignmentService
{
    ...
    public async Task<AssignmentResponse> UpdateAssignmentAsync(string staffCode, UpdateAssignmentRequest assignmentRequest)
    {
        var staff = await _userRepository.GetByIdAsync(staffCode);
        if (staff is null)
        {
        ...
        var assignment = await _assignmentRepository.GetByIdAsync(assignmentRequest.Id);
        ...
        if (staff.Location != assignment.Asset.Location)
        {
            var attributes = new Dictionary<string, object>
            {
                { "location", assignment.Asset.Location.ToString() }
            };
        ...
        if (assignment.AssignedTo != assignmentRequest.StaffCode)
        {
            ...
            if (newAssignedTo.Location != staff.Location &&
                newAssignedTo.Location != assignment.Asset.Location)
            {
                var attributes = new Dictionary<string, object>
                {
